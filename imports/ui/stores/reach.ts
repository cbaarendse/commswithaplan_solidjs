...
export const deployedTouchPointsForFormula = derived(touchPointsDefinitions, ($touchPointsDefinitions) =>
  $touchPointsDefinitions.map((touchPointDefinition) => {
    return {
      ...touchPointDefinition,
      value: <Writable<number>>writable(0.0),
      show: <Writable<boolean>>writable(true),
      inputType: <Writable<InputType>>writable('reach')
    };
  })
);

export const deployedTouchPointsForData = derived(
  ...
  ([$touchPointsDefinitions, $touchPointsPerInputType]) =>
    $touchPointsDefinitions.map((touchPointDefinition) => {
      return {
        ...
        value: <Writable<number>>writable(0.0),
        show: <Writable<boolean>>writable(true),
        inputType: <Writable<InputType>>(
          writable(reachTool.setDefaultInputType(touchPointDefinition.name, $touchPointsPerInputType))
        )
      };
    })
);

export const defaultFormulaStrategy = derived(deployedTouchPointsForFormula, ($deployedTouchPointsForFormula) => {
  return {
    userId: '',
    title: 'New Strategy',
    marketName: 'nl',
...
export const defaultDataStrategy = derived(deployedTouchPointsForData, ($deployedTouchPointsForData) => {
  return {
    userId: '',
    title: 'New Strategy',
    marketName: 'nl',
