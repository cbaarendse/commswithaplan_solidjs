...
export const touchPointsDefinitions: TouchPointDefinition[] = [
  {
    name: 'advocacy',
    definitions: [
      {
...
export const touchPointsPerInputType: {[key: string]: string[]} = {
  contacts: [
    'ambassador',
    'direct_mail',
    'door_drop',
...
export const deployedTouchPointsForFormula = writable(
...
export const deployedTouchPointsForData = writable(
  touchPointsDefinitions.map((touchPointDefinition) => {
    return {
      ...touchPointDefinition,
      value: 0.0,
...
export const strategyWithFormula = {
  marketName: 'nl',
  marketData: false,
  userId: '',
  title: 'New Strategy',
...
export const strategy = writable(marketData && useMarketData ? strategyWithData : strategyWithFormula);
export const strategyDerived = derived(
  [marketData, useMarketData, marketName],
  ([$marketData, $useMarketData, $marketName]) => {
    if ($marketData && $useMarketData) {
      strategyWithData.marketData = $marketData;
      strategyWithData.useMarketData = $useMarketData;
      strategyWithData.marketName = $marketName;
      return strategyWithData;
    } else {
      strategyWithFormula.marketData = $marketData;
      strategyWithFormula.marketName = $marketName;
      return strategyWithFormula;
    }
  }
);
