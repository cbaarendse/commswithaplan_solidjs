import '../imports/both/constants';
import './fixtures';

import '../imports/api/users/server/publications.ts';
// import '../imports/api/users/server/users_payments';
import '../imports/api/users/methods.ts';

// import '../imports/api/notifications/server/publications';
//import '../imports/api/notifications/methods';

// import '../imports/api/brands/server/publications';
// import '../imports/api/brands/methods';

// import '../imports/api/campaigns/server/publications';
// import '../imports/api/campaigns/methods';

// import '../imports/api/companies/server/publications';
// import '../imports/api/companies/methods';

// import '../imports/api/emails/server/publications';
//import '../imports/api/emails/methods';

// import '../imports/api/strategies/server/publications';
// import '../imports/api/strategies/methods';

// import '../imports/api/populations/server/publications';
// import '../imports/api/populations/methods';

// import '../imports/api/probabilities/server/functions';
// import '../imports/api/probabilities/methods.js';

// import '../imports/api/products/server/publications.js';
// import '../imports/api/products/methods.js';

// import '../imports/api/touchpoints/server/publications.js';
// import '../imports/api/touchpoints/methods.js';

// import '../imports/api/content/server/publications.ts';
// import '../imports/api/content/methods.ts';

// import '../imports/api/touchpointsbasics/server/publications.ts';
// import '../imports/api/touchpointsbasics/methods.ts';

// import '../imports/api/translations/server/publications.ts';
// import '../imports/api/translations/methods.js';
